...
class Human {
  ...
  void track() {
    ...
    for (int i=0; i < depth.length; i++) {
      ...
      //find human
      if (rawDepth >= thresholdMin
        && rawDepth <= thresholdChairMin
        && rawDepth != 0) {
    ...
    //println(chairCount);
    if (chairCount > chairConfirmationMin && chairCount < chairConfirmationMax
  ...
  void display() {
    ...
    for (int x = 0; x < kinect2.depthWidth; x++) {
      for (int y = 0; y < kinect2.depthHeight; y++) {
        ...
        if (rawDepth > 0 && rawDepth >= thresholdChairMin && rawDepth <= thresholdChairMax
          && y > chairTopY && x > chairMinX && x < chairMaxX) {
          // A red color instead
