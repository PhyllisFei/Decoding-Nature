class Boid {
  ...
  void run(ArrayList<Boid> boids) {
    ...
    update();
    borders();
    fixOverlapping(boids);
    render();
    a += .01;
  }
  
  void fixOverlapping(ArrayList<Boid> boids){
    for(Boid b: boids){
      if(abs(this.position.copy().sub(b.position.copy()).mag()) < 1.4){
        this.position.add(PVector.random2D());
      }
    }
  }
  ...
  void connect2(ArrayList<Boid> others) {
